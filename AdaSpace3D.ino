...
#define PREVENTIVE_RESET_INTERVAL                                              \
  0 // Set to 300000 (5 mins) if you want periodic resets

// --- LED SETUP ---
Adafruit_NeoPixel strip(NUM_ADDRESSABLE_LEDS, PIN_NEOPIXEL,
                        NEO_GRB + NEO_KHZ800);
...
bool streaming = false; // Control for live data streaming

// HID Report Descriptor
static const uint8_t spaceMouse_hid_report_desc[] = {
    0x05, 0x01, 0x09, 0x08, 0xA1, 0x01, 0xA1, 0x00, 0x85, 0x01, 0x16,
    0x00, 0x80, 0x26, 0xFF, 0x7F, 0x36, 0x00, 0x80, 0x46, 0xFF, 0x7F,
    0x09, 0x30, 0x09, 0x31, 0x09, 0x32, 0x75, 0x10, 0x95, 0x03, 0x81,
    0x02, 0xC0, 0xA1, 0x00, 0x85, 0x02, 0x16, 0x00, 0x80, 0x26, 0xFF,
    0x7F, 0x36, 0x00, 0x80, 0x46, 0xFF, 0x7F, 0x09, 0x33, 0x09, 0x34,
    0x09, 0x35, 0x75, 0x10, 0x95, 0x03, 0x81, 0x02, 0xC0, 0xA1, 0x00,
    0x85, 0x03, 0x05, 0x09, 0x19, 0x01, 0x29, 0x20, 0x15, 0x00, 0x25,
    0x01, 0x75, 0x01, 0x95, 0x20, 0x81, 0x02, 0xC0, 0xC0};
...
void setup() {
  ...
  Serial.begin(115200); // Always init for commands/streaming

  for (uint8_t i = 0; i < 4; i++)
    pinMode(physPins[i], INPUT_PULLUP);

  TinyUSBDevice.setID(USB_VID, USB_PID);
  usb_hid.setReportDescriptor(spaceMouse_hid_report_desc,
                              sizeof(spaceMouse_hid_report_desc));
...
void loop() {
  handleSerialConfig(); // Handle serial commands
...
void handleSerialConfig() {
  if (Serial.available()) {
    ...
    if (cmd == "STREAM:ON") {
      streaming = true;
      Serial.println("OK_STREAM");
    } else if (cmd == "STREAM:OFF") {
      streaming = false;
      Serial.println("OK_STREAM_OFF");
...
void readAndSendMagnetometerData() {
  ...
  if (PREVENTIVE_RESET_INTERVAL > 0 &&
      millis() - watchdog.lastPreventiveReset > PREVENTIVE_RESET_INTERVAL) {
    resetMagnetometer();
    watchdog.lastPreventiveReset = millis();
    return;
  }
  ...
  if (activeSensor->getMagneticField(&x, &y, &z)) {
    if (isfinite(x) && isfinite(y) && isfinite(z)) {
      ...
      if (x == watchdog.last_x && y == watchdog.last_y &&
          z == watchdog.last_z) {
      ...
      // Stream data for live visualization
      if (streaming) {
        ...
        if (millis() - lastStream > 50) { // 20Hz limit
          ...
          Serial.print(z, 4);
          Serial.print(",");
          Serial.print(currentButtonState[0]);
          Serial.print(",");
          Serial.print(currentButtonState[1]);
          Serial.print(",");
          Serial.print(currentButtonState[2]);
          Serial.print(",");
...
void send_tx_rx_reports(int16_t tx, int16_t ty, int16_t tz, int16_t rx,
                        int16_t ry, int16_t rz) {
  if (!TinyUSBDevice.mounted() || !usb_hid.ready())
    return;

  uint8_t tx_report[6] = {(uint8_t)tx, (uint8_t)(tx >> 8),
                          (uint8_t)ty, (uint8_t)(ty >> 8),
                          (uint8_t)tz, (uint8_t)(tz >> 8)};
  usb_hid.sendReport(1, tx_report, 6);

  delayMicroseconds(500);

  uint8_t rx_report[6] = {(uint8_t)rx, (uint8_t)(rx >> 8),
                          (uint8_t)ry, (uint8_t)(ry >> 8),
                          (uint8_t)rz, (uint8_t)(rz >> 8)};
